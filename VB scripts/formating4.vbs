...
Sub TransformData()
    ...
    Dim currentProjectManager As String
    Dim lastProjectManager As String
    ...
    headers = Array("שם מנהל פרויקט", "קוד חשבון", "שם חשבון", "סעיף תקציבי", "סעיף תקציבי נוסף", _
                   "אסמכתא", "תאריך אסמכתא", "תאריך לתשלום", "תאריך פעילות", "Status", "Sum of יתרה")
    
    With outputSheet.Range("A1").Resize(1, UBound(headers) + 1)
        .Value = headers
        .Font.Bold = True
    ...
    For Each groupKey In groupedData.Keys
        ...
        For Each subGroupKey In groupedData(groupKey).Keys
            For Each rowKey In groupedData(groupKey)(subGroupKey).Keys
                ...
                With outputSheet
                    ' Only write project manager name if it's different from the last one
                    If projectManager <> lastProjectManager Then
                        .Cells(outputRow, 1).Value = projectManager
                        lastProjectManager = projectManager
                    Else
        ...
        With outputSheet
            .Cells(outputRow, 3).Value = accountCode & " Total"
        ...
        lastProjectManager = "" ' Reset for next group
    Next groupKey
